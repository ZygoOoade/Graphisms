...
async def generate_flux_image(prompt: str):
    try:
        ...
        image_path = result[0] if isinstance(result, tuple) else result
        return [image_path], "FLUX.1-schnell"
    except Exception as e:
        print(f"Error generating FLUX.1-schnell image: {e}")
        return None, "FLUX.1-schnell"
...
async def askai(ctx: commands.Context, *, prompt: str):
    initial_message = await ctx.reply("Generating images from Midjourney, DALL-E, Stable Diffusion, PixArt-alpha, and FLUX.1-schnell, please wait...")
